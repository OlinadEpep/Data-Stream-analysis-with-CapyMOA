...
def run_srp(chunk_paths, chunk_ids, result_dirs):
    """
    Esegue Streaming Random Patches (SRP10) in modo continuo sui chunk etichettati al 100%.
    Versione con doppio grafico diagnostico PCA (Realtà vs Predizioni).
    """
    print(f"\n🚀 Esecuzione SRP10 continua su {len(chunk_paths)} chunk...")
    ...
    # Conserviamo gli ID per il salvataggio finale, ma li rimuoviamo dal dataset di training
    ...
    # 2. Creazione dello Stream CapyMOA
    ...
    # Mappiamo le classi in indici contigui (0, 1, 2...) per evitare 'list index out of range'
    # in CapyMOA/MOA quando mancano classi intermedie nello Scenario 4.
    classes = np.unique(y_raw[y_raw != -1])
    y = np.copy(y_raw)
    for i, c in enumerate(classes):
        y[y_raw == c] = i

    # Scaling dei dati solo per la PCA e fit globale
    ...
    stream = NumpyStream(
        X=X, 
        y=y, 
        dataset_name="stream_continuo",
        feature_names=feature_cols,
        target_name="Classe"
    )

    # 3. Inizializzazione Modello SRP (10 estimators)
    model = StreamingRandomPatches(schema=stream.get_schema(), ensemble_size=10)
    ...
    # 4. Prequential Evaluation Loop (Test-Then-Train)
    while stream.has_more_instances():
        instance = stream.next_instance()
        
        # Estrarre il target reale mappato
        ...
        if 0 <= y_true_idx < len(classes):
            ...
            y_true = -1
            is_labeled = False  # L'istanza fa parte della percentuale NON etichettata

        # TEST: Il modello fa sempre la predizione (serve anche per il grafico PCA finale)
        ...
        # 🚨 METRICHE: Aggiorna i contatori SOLO se l'istanza ha un'etichetta reale!
        if is_labeled:
            if y_pred == y_true:
                correct += 1
            total += 1

        # TRAIN: Comportamento dinamico basato sul tipo di modello
        if is_labeled:
            # Se c'è l'etichetta, tutti i modelli si allenano normalmente
            model.train(instance)
        else:
            # Se NON c'è l'etichetta, controlliamo se il modello supporta l'apprendimento non supervisionato
            if hasattr(model, 'train_on_unlabeled'):
                model.train_on_unlabeled(instance)
            else:
                pass # Un modello supervisionato puro (HAT, AUE) salta l'addestramento se manca il target
        ...
        if instances_processed_in_chunk == chunk_sizes[current_chunk_idx]:
            ...
            # Filtriamo via i dati non etichettati prima di passarli a Scikit-Learn
            ...
            print(f"✅ Chunk {chunk_id} Completato! Acc: {acc:.4f} | Prec: {prec_macro:.4f} | Rec: {rec_macro:.4f} | F1: {f1_macro:.4f} | Kappa: {kappa:.4f} (Tempo: {end_time - start_time:.2f}s)")
            ...
            # Salva i risultati riga per riga per la tesi (solo per il chunk corrente)
            ...
            # ==========================================
            # NUOVO BLOCCO GRAFICO PCA: REALTÀ VS PREDIZIONI
            # ==========================================
            ...
            # Ordina per stampare prima gli eventuali -1 (grigi) per non coprire i punti colorati
            ...
            palette_reale[-1] = "lightgray" # Per SRP potrebbe non esserci, ma gestiamo il caso generico
            ...
            axes[1].set_title(f"Predizioni Modello SRP - Chunk {chunk_id}")
            axes[1].set_xlabel(xlabel_str)
            axes[1].set_ylabel(ylabel_str)

            # Limiti fissi per mantenere il grafico coerente tra i chunk
