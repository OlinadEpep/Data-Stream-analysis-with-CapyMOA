...
try:
    from capymoa.classifier import OzaBoostAdwin
except ImportError:
    from capymoa.classifier import OnlineAdwinBagging as OzaBoostAdwin
...
def run_ozaboost_adwin(chunk_paths, chunk_ids, result_dirs):
    print(f"\n🚀 Esecuzione OzaBoostADWIN continua su {len(chunk_paths)} chunk...")
    df_list = []
    chunk_sizes = []
    ...
    stream = NumpyStream(
        X=X, 
        y=y, 
        dataset_name="stream_continuo_ozaboost",
        feature_names=feature_cols,
        target_name="Classe"
    )

    model = OzaBoostAdwin(schema=stream.get_schema())
    ...
    while stream.has_more_instances():
        ...
        if instances_processed_in_chunk == chunk_sizes[current_chunk_idx]:
            ...
            axes[1].set_title(f"Predizioni Modello OzaBoostADWIN - Chunk {chunk_id}")
